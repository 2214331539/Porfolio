...
export type SiteContextValue = { settings: SiteSettings | null; settingsLoading: boolean; settingsError: string | null; theme: Theme; toggleTheme: () => void };
export const SiteContext = createContext<SiteContextValue>({ settings: null, settingsLoading: true, settingsError: null, theme: 'light', toggleTheme: () => undefined });
...
export function SiteProvider({ children }: { children: ReactNode }) {
  ...
  const value = useMemo(() => ({ settings, settingsLoading, settingsError, theme, toggleTheme: () => setTheme((current) => current === 'light' ? 'dark' : 'light') }), [settings, settingsLoading, settingsError, theme]);
